...
class Api {
    ...
    **/
    function deleteApiParam($apiParamId) {
        ...
        if (!empty($result)) {
            if($result) {
                return self::getApiParameterData();
            }
            else
                return array('status' => "error", 'code' => 1, 'statusMsg' => 'Failed to delete', 'data' => '');
        }else{
    ...
    **/
    public function getOneApi($command) {
        $db = MysqliDb::getInstance();
        $db->where("site",Setting::$accessUser['site']);
        ...
        if(empty($result)){
            $insertAPI = array(
                "command" => $command,
                "description" => $command,
                "site" => Setting::$accessUser['site'],
                "duration" => 60,
                "no_of_queries" => 100,
                "check_duplicate" => 0,
                "check_duplicate_interval" => 1,
                "created_at" => "2020-09-09 09:09:09",
            );

            $db->insert("api",$insertAPI);
    ...
    function apiParamsScriptCheck($array, $check) {
        $db = MysqliDb::getInstance();
        if($check) {
            foreach($array as $key => $val) {
                if(is_array($val)) {
                    if($check)
                        self::apiParamsScriptCheck($val,$check);
                } else {
    ...
    function checkApiParams($apiID, $params) {
        ...
        $check = TRUE;
        $check = self::apiParamsScriptCheck($params, $check);
        if(!$check) {
            return array('status' => 'error', 'code' => 1, 'statusMsg' => 'Invalid value detected.', 'data'=>'');
        }
        ...
        $db->where('command', 'addTicket');
        $addTicketID = $db->getValue('api', 'id');

        if($apiID == 149){
        	unset($tmpA);
        	$tmpA = array("senderId", "ticketId", "message");
        } else if($apiID == $addTicketID) {
            unset($tmpA);
            $tmpA = array("clientID", "subject", "message", "type");
        }
        ...
        foreach($result as $array) {
            ...
            if(is_array($value)) {
                $check = TRUE;
                self::apiParamsArrayCheck($value, $paramsType, $compulsory, $check);
                if(!$check) {
                    $data['type'] = $webType;
                    $data['field'] = $array['params_name'];
//                            $data['compulsory'] = $compulsory;
                    if($array['params_name'] == 'fileName'){
                        return array('status' => 'error', 'code' => 1, 'statusMsg' => 'File name contains special characters.', 'data'=>$data);
                    }else{
                ...
                $check = self::paramsChecker($value, $paramsType, $compulsory);
                if(!$check) {
                    $data['type'] = $webType;
                    $data['field'] = $array['params_name'];
//                        $data['compulsory'] = $compulsory;
        ...
        if(strlen($value) == 0) {
            ...
            switch($paramsType) {
                ...
                case "alphanumeric":
                    ...
                    break;
                case "general":
                    $specialChars = self::getSpecialCharacters();
    ...
    function checkApiDuplicate($tblDate, $createTime, $userID, $sessionID, $site, $command, $duplicateInterval) {
        ...
        if($tblDate == $wsDateCurr) {
            $wsResult = self::wsDataIn($userID, $site, $command, $tsFrom, $createTime, $wsDateCurr);
        }
        ...
        $wsResult = self::wsDataIn($userID, $site, $command, $tsFrom, $createTime, $wsDatePrev);
        $wsResRows = count($wsResult);
        if($wsResRows <= 1) {
            return array('status' => 'ok', 'code' => 0, 'statusMsg' => '', 'data'=>''); // return here no need to perform further checking
        }
    ...
    function wsDataIn($userID, $site, $command, $timeFrom, $timeTo, $wsDate) {
        ...
        $wsResult = $db->get('web_services_'.$wsDate, null, 'data_in,status');
        return $wsResult;
    }
    ...
    function setSpecialCharacters() {
        ...
        $db->where('disabled', 0);
        self::$specialCharacters = $db->get('special_characters', null, 'value');
    }
    ...
    function apiParamsArrayCheck($array, $paramsType, $compulsory, &$check) {
        ...
        if($check) {
            foreach($array as $key => $val) {
                if(is_array($val)) {
                    if($check)
                        self::apiParamsArrayCheck($val, $paramsType, $compulsory, $check);
                } else {
                    $check = self::paramsChecker($val, $paramsType, $compulsory);
                    if(!$check)
                        continue;
                }
            }
	...
	function loopReplace(&$item, $keyAry){
        foreach($item as $key => &$value){
        	if(is_array($value)) {
                self::loopReplace($value, $keyAry);
            }else if(in_array($key, $keyAry)){
				$value = "base64";
            }
        }
    }
    ...
    //check html tag
    function apiParamsTagCheck($array,&$check) {
        $db = MysqliDb::getInstance();
        if($check) {
            foreach($array as $key => $val) {
                if(is_array($val)) {
                    if($check)
                        self::apiParamsTagCheck($val, $check);
                } else {
                    if($val != strip_tags($val)){
                        $check = false;
                        break;
