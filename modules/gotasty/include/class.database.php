...
class MysqliDb
{
    ...
    /**
     * Variable that holds total number of queries
     *
     * @var int
     */
    ...
    public static function getFilterInstance()
    {
    	if(self::$_instance2){
        	return self::$_instance2;
    	}else{
	    	self::$_instance2 = self::$_instance->copy();
	    	self::$_instance2->resetState();
    	}
        return self::$_instance2;
    }
     ...
     * @amendment Added the Journal Log Function on 14/08/2017.
     */
    public function insert($tableName, $insertData)
    {
        $userID = 1;
        $this->queryNumber++;
        $insertResult = $this->_buildInsert($tableName, $insertData, 'INSERT');
        
        //Journal Log
        if($insertResult) {
            $insertData["id"] = $insertResult;
            $this->journalLog($tableName, "insert", $insertData, NULL);
        }
    ...
    {   
        ...
        // Build the old data to be used for journals
        $oldData = $this->_buildOldData($tableName, $numRows);
        ...
        if ($stmt->affected_rows > 0) {
            // Journal Log.
            $this->journalLog($tableName, "update", $tableData, $oldData);
        }

        return $status;
    }
    
    private function _buildOldData($tableName, $numRows)
    {
        $copyDb = $this->copy();
        $copyDb->_query = "SELECT * FROM $tableName ";
        $copyDb->_buildCondition('WHERE', $copyDb->_where);
        $copyDb->_buildLimit($numRows);
        $copyDb->_lastQuery = $copyDb->replacePlaceHolders($copyDb->_query, $copyDb->_bindParams);
        $res = $copyDb->rawQuery($copyDb->_lastQuery);
    ...
    public function delete($tableName, $numRows = null)
    {
        ...
        if ($stmt->affected_rows > 0){
            $this->journalLog($tableName, "delete", NULL, $oldData);
        }
...
//        }
    ...
    /**
     * This methods returns the number of queries
     *
    ...
    {
        $data = array("created_at" => date("Y-m-d H:i:s"));
        $id = $this->insert("transaction_id", $data);
        return $id;
    }
    ...
    protected function _bindParam($value)
    {
        ...
        array_push($this->_bindParams, trim($value));
    }
    ...
    /**
     * Abstraction method that will ONLY build the part of the WHERE conditions
     *
     * @param string $operator
     * @param array $conditions
     */
    protected function _buildCustomCondition($operator, $conditions)
    {
        if (empty($conditions)) {
            return;
        }
        ...
        foreach ($conditions as $cond) {
            ...
            switch (strtolower($operator)) {
                ...
                case 'in':
                    $query .= ' ' . $operator . ' (';
                    if (is_object($val)) {
                        // Not handled for object, please use array
                    } else {
                        $query .= "'".implode("','", $val)."'";
                    }
                    $query .= ')';
                ...
                case 'not between':
                case 'between':
                    $query .= " $operator ".$val[0]." AND ".$val[1];
                    break;
                case 'not exists':
                case 'exists':
                    // Not handled for exists/not exists, please use another operator
                    break;
                default:
                    if (is_array($val)) {
                        $query .= ' ' . $operator . " '".$val[0]."'";
                        unset($val[0]);
                        foreach ($val as $v) {
                            $query .= ' ' . $concat . ' ' . $varName . ' ' . $operator . " '".$v."'";
                        }
                    } elseif ($val === null) {
                        $query .= ' ' . $operator . " NULL";
                    } elseif ($val != 'DBNULL' || $val == '0') {
    ...
    /**
     * Performs the Journal log of Create/Edit/Delete.
     * @param  String   $tableName    The table name to be create.
     * @param  String   $method       Database Action.
     * @param  Array    $newData      Database Fields and Data.
     * @param  Array    $oldData      Database Old Fields
     */
    public function journalLog($tableName = NULL, $method = NULL, $newData = NULL, $oldDataArray = NULL) {
        
        $journalTableResult = $this->rawQuery("SELECT table_name, type, disabled FROM journal_tables WHERE table_name = '". $tableName. "' AND disabled = 0 AND deleted = 0");
        if (count($journalTableResult) == 0) {
            // If table is not in journal tables list, do nothing
            return;
        }
        
        if ($journalTableResult[0]['disabled'] == 1) {
            // If table is disabled from journal, do nothing
            return;
        }
        
        if(strpos($tableName, '_audit') !== FALSE) {
            // If it's an insertion or update from another audit table, do nothing
            return;
        }
        
        // check if table and column can skip insert record
        $skipColArr = $this->skipJournalLog($tableName,$newData);

        $i = 0;
        $this->rawQuery("CREATE TABLE IF NOT EXISTS ".$tableName."_audit LIKE audit");
        
        if ($newData) {
            
            foreach ($newData as $newKeys => $newValues) {
                // skip if in skip array
                if(in_array($newKeys, $skipColArr)) continue;
                
                if (strtolower($method) == 'insert') {
                    
                    $totData[$i]['new_value'] = $newValues;
                    $totData[$i]['method'] = $method;
                    $totData[$i]['row_id'] = $newData['id'];
                    $totData[$i]['old_value'] = "";
                    $totData[$i]['column_name'] = $newKeys;
                    $totData[$i]['created_by'] = $this->userID? $this->userID : 0;
                    $totData[$i]['created_type'] = $this->userType? $this->userType : "System";
                    $totData[$i]['created_at'] = date("Y-m-d H:i:s");
                    $totData[$i]['updated_at'] = date("Y-m-d H:i:s");
                    $i++;
                    
                }
                else if (strtolower($method) == 'update') {
                    
                    foreach ($oldDataArray as $oldData) {
                        
                        if ($newValues == $oldData[$newKeys]) {
                        ...
                        $totData[$i]['new_value'] = $newValues;
                        $totData[$i]['method'] = $method;
                        $totData[$i]['row_id'] = $oldData['id'];
                        $totData[$i]['old_value'] = $oldData[$newKeys];
                        $totData[$i]['column_name'] = $newKeys;
                        $totData[$i]['created_by'] = $this->userID? $this->userID : 0;
                        $totData[$i]['created_type'] = $this->userType? $this->userType : "System";
                        $totData[$i]['created_at'] = date("Y-m-d H:i:s");
            ...
            if ($method == 'delete') {
                ...
                foreach ($oldDataArray as $oldData) {
                    ...
                    foreach ($oldData as $oldKeys => $oldValues) {
                        $totData[$i]['new_value'] = "";
                        $totData[$i]['method'] = $method;
                        $totData[$i]['row_id'] = $oldData['id'];
                        $totData[$i]['old_value'] = $oldValues;
                        $totData[$i]['column_name'] = $oldKeys;
                        $totData[$i]['created_by'] = $this->userID? $this->userID : 0;
                        $totData[$i]['created_type'] = $this->userType? $this->userType : "System";
                        $totData[$i]['created_at'] = date("Y-m-d H:i:s");
                        $totData[$i]['updated_at'] = date("Y-m-d H:i:s");
        ...
        if (!empty($totData))
            $result = $this->insertMulti($tableName."_audit", $totData);
        
        // Remove the query count additions from journals
        $this->queryNumber -= (2 + $i);
        
        return $result;
    }


    function skipJournalLog($tableName,$data){
    	switch ($tableName) {
    		case 'client':
    			$skipArr = array("last_activity","updated_at","session_id","fail_login");
    ...
    /**
     * This function is used for Password Encryption in Mysql Format.
     * @param $password Password String.
     *
     **/
    public function encrypt($password) {
        $encryptedPassword = $this->rawQueryOne("SELECT PASSWORD('$password') AS password;");
        return $encryptedPassword["password"];
    }
    
    /**
     * This function is used for Optimizing table after deletion.
     * @param string  $tableName The name of the database table to work with.
     *
     **/
    public function optimize($tableName) {
        $this->rawQuery("OPTIMIZE TABLE ".$tableName);
    }
